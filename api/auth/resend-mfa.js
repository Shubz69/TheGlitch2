...
module.exports = async (req, res) => {
  ...
  try {
      ...
      if (db) {
        try {
          ...
          if (userRows && userRows.length > 0) {
            userEmail = userRows[0].email.toLowerCase();
          }
          await db.end();
        } catch (error) {
    ...
    if (db) {
      try {
        ...
        if (userId) {
          ...
          await db.execute('DELETE FROM mfa_codes WHERE email = ?', [userEmail]);
        }
        
        // Insert new code
        await db.execute(
          'INSERT INTO mfa_codes (user_id, email, code, expires_at) VALUES (?, ?, ?, ?)',
          [userId || null, userEmail, code, expiresAt]
        );
        await db.end();
      } catch (dbError) {
    ...
    const mailOptions = {
      ...
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          ...
          <p>Your new MFA verification code is:</p>
    ...
    console.log(`MFA code resent to ${userEmail}`);
    ...
    return res.status(500).json({ 
      success: false, 
      message: 'Failed to resend MFA code. Please try again later.' 
    });
  }
};
